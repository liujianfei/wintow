 ...
 */
function get_apply_list($user_id, $user_name, $num, $start, $order_id = 0)
{
    /* 获取留言数据 */
    $msg = array();
    ...
    {
        $sql .= " WHERE parent_id = 0 AND user_id = '$user_id' AND msg_type=6 AND user_name = '" . $_SESSION['user_name'] . "' AND order_id=0 ORDER BY msg_time DESC";
    }
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line,user_type, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
