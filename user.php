...
'message_list','apply_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
...
elseif ($action == 'profile')
{
    ...
    $smarty->assign('shop_province_list', get_regions(1, $_CFG['shop_country']));
	 ...
	 $sql = "SELECT country,province,city,district FROM " . $GLOBALS['ecs']->table('users') .
            " WHERE user_id = '$user_id' ";

    $row=$GLOBALS['db']->getRow($sql);
	
$consignee_list[] = array('country' => $_CFG['shop_country'],'province' => $row['province'],'city' => $row['city'],'district' => $row['district'], 'email' => isset($_SESSION['email']) ? $_SESSION['email'] : '');
...
elseif ($action == 'act_edit_profile')
{
    ...
    if (edit_profile($profile))
    {
	  ...
	  $address=trim($_POST['address']);
		...
		$card_type=isset($_POST['cardtype']) ? intval($_POST['cardtype']):0;
		...
		if($_POST['user_types']==2)
		{
		
		$sql = 'UPDATE ' . $ecs->table('users') . " SET company_name='$company_name',card_type='$card_type'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";

        $db->query($sql);
		
		}
	
	    $sql = 'UPDATE ' . $ecs->table('users') . " SET  real_name='$real_name' ,country='$country',province='$province' , city='$city' , district='$district', address='$address'  WHERE `user_id`='" . $_SESSION['user_id'] . "'";
...
elseif ($action == 'apply_list')
{
    ...
    if ($order_id)
    {
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('feedback').
                " WHERE parent_id = 0 AND order_id = '$order_id' AND user_id = '$user_id' AND msg_type=10";
    ...
    {
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('feedback').
           " WHERE parent_id = 0 AND user_id = '$user_id' AND user_name = '" . $_SESSION['user_name'] . "' AND order_id=0 AND msg_type=6";
    }
    ...
    $smarty->assign('apply_list', get_apply_list($user_id, $_SESSION['user_name'], $pager['size'], $pager['start'], $order_id));
    $smarty->assign('pager',        $pager);
...
elseif ($action == 'comment_list')
{
    ...
    $record_count = $db->getOne($sql);
    $pager = get_pager('user.php', array('act' => $action), $record_count, $page, 2);
...
elseif ($action == 'act_add_message')
{
    ...
    {
        show_message($_LANG['add_apply_success'], $_LANG['apply_list_lnk'], 'user.php?act=apply_list&order_id=' . $message['order_id'],'info');
    }
    else
    {
        $err->show($_LANG['apply_list_lnk'], 'user.php?act=apply_list');
    }
}
...
elseif ($action == 'message_list')
{
    ...
    {
        $sql = "SELECT COUNT(*) FROM " .$ecs->table('feedback').
                " WHERE parent_id = 0 AND order_id = '$order_id' AND user_id = '$user_id' AND msg_type <>6";
        $order_info = $db->getRow("SELECT * FROM " . $ecs->table('order_info') . " WHERE order_id = '$order_id' AND user_id = '$user_id'");
        $order_info['url'] = 'user.php?act=order_detail&order_id=' . $order_id;
    }
    else
...
elseif ($action == 'del_app')
{
    ...
    ecs_header("Location: user.php?act=apply_list&order_id=$order_id\n");
    exit;
}
